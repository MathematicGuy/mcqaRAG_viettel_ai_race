"""
Llama.cpp server client for answer generation.
...
class OllamaCppClient:
    """
    Llama.cpp server client for MCQ answer generation.
    Uses OpenAI-compatible /v1/chat/completions endpoint.
    """

    def __init__(
        self,
        host: str = "http://127.0.0.1:8082",
        model: str = "Qwen/Qwen3-4B-GGUF:Q8_0",
        ...
        Args:
            ...
            max_response_words: Maximum words in responseA
        ...
        self.logger.info(f"Llama.cpp client initialized: {host}, model: {model}")
    ...
    def _build_mcq_prompt(
        ...
        3. Select at least ONE correct answer, only choose TWO correct answer if enough context.
        4. Provide brief reasoning\n\n

        OUTPUT FORMAT (REQUIRED - Always provide all 3 fields):
        ANSWER: [Single option: A or B or C or D]
        REASONING: [Brief explanation of your choice]
        CONFIDENCE: [high/medium/low]\n\n
        RULES:
        - Use ONLY information from context
        - If no relevant context, use your best judgment and say "low" confidence
        - Keep reasoning concise
        - Always provide all 3 output fields in the exact format above
        """
            ...
            for chunk in doc_data["chunks"]:
                ...
                if first_newline_pair_idx != -1:
                    ...
                    if header_line.startswith(expected_header_prefix):
                        cleaned_chunk_text = full_chunk_text[first_newline_pair_idx + 4:].strip()

                section = chunk.get("section_name", "Unknown")
                if section == "Full Document":
                    chunk_content_for_prompt = cleaned_chunk_text
                else:
    ...
    def _parse_mcq_response(self, response: str) -> Dict:
        """
        Parse LLM response to extract answer, reasoning, and confidence.

        Handles both single-line and multi-line REASONING formats:
        - Single-line: REASONING: Brief explanation
        ...
        while i < len(lines):
            ...
            # ✅ Parse REASONING field (handle multi-line format)
            elif line.startswith("REASONING:"):
                reasoning_parts = []

                # Get text on same line as REASONING:
                same_line_text = line.split(":", 1)[-1].strip()
                if same_line_text:
                    reasoning_parts.append(same_line_text)

                # Collect all lines until we hit CONFIDENCE: or end of response
                i += 1
                while i < len(lines):
                    next_line = lines[i].strip()

                    # Stop at CONFIDENCE
                    if next_line.startswith("CONFIDENCE:"):
                        break
                ...
                # (we already moved past REASONING section)
                continue
            ...
            # ✅ Parse CONFIDENCE field
                ...
                if confidence in ["high", "medium", "low"]:
                    ...
                    self.logger.warning(f"Invalid confidence value: {confidence}, defaulting to 'medium'")
        ...
        # ✅ Fallback: If no ANSWER found, try to extract first letter
        if not result["option"]:
            for char in ["A", "B", "C", "D"]:
                if response.upper().find(char) != -1:
                    result["option"] = char
                    result["confidence"] = "low"
                    self.logger.warning(f"No valid ANSWER found in response, extracted: {char}")
                    break

        # ✅ Final validation
        if not result["option"]:
            result["option"] = "C"  # Default fallback
            result["confidence"] = "low"
            self.logger.warning("No valid answer extracted, defaulting to: C")

        return result

    async def _generate(self, prompt: str) -> Dict:
        """
        Call llama.cpp server's /v1/chat/completions API.
        ...
        payload = {
            ...
            "max_tokens": self.max_response_words * 2,  # Words to tokens approximation
            "temperature": self.temperature,
        }

        async with httpx.AsyncClient(timeout=self.timeout) as client:
            response = await client.post(url, json=payload)
            self.logger.debug(f"Raw llama.cpp response: {response.text}")
            ...
            if "choices" in data and len(data["choices"]) > 0:
                ...
                raise ValueError("Invalid response format from llama.cpp server")
    ...
    async def generate_stream(self, prompt: str) -> AsyncGenerator[str, None]:
        ...
        payload = {
            ...
            "max_tokens": self.max_response_words * 2,
            "temperature": self.temperature,
            "stream": True,
        }
    ...
    async def check_model(self) -> bool:
        """
        Check if model is available (simple health check).
    ...
    def health_check(self) -> Dict:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return {"status": "unhealthy", "error": "Cannot connect to llama.cpp server"}
